...
**Enterprise plans:** If your organization is on an Anthropic Enterprise usage-based plan, the same Claude CLI auth is used. The plugin automatically detects the monthly dollar spend limit from the OAuth usage endpoint and shows a `Monthly:` entry instead of (or alongside) the standard 5h/Weekly token windows.
...
| Enterprise `Monthly:` entry missing | Confirm your org is on a usage-based Enterprise plan and that `extra_usage.is_enabled` is true in the raw API response. Re-authenticate Claude if the OAuth credential is stale. |
