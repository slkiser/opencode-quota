...
| Native TUI `/quota`            | `tuiQuotaCommandDisplay: "inline"` or `"dialog"`                                          | `inline` is the default and adds ignored/no-reply plain text to the transcript. `dialog` keeps the local popup. Neither mode calls a model.                                                                        |
| Slash and palette commands     | TUI plugin in `tui.json`; server plugin in `opencode.json`                                | OpenCode 1.18.2 registers each command once. `/quota_status` and `/quota_announcements` keep TUI dialogs; Web/Desktop gets deterministic ignored/no-reply plain text.                                              |
...
In OpenCode 1.18.2, `/quota` works in TUI and Web without calling a model. Native TUI output enters the transcript by default with `noReply` and `ignored`; choose `"dialog"` for the existing local popup. This setting does not change `/quota_status` or `/quota_announcements`. Web returns clean plain text and can show a false `Failed to send command` notification after the deterministic output appears; do not retry automatically after output is visible. OpenCode 1.18.2 has no clean server-command cancellation API. Safari/macOS notification permissions do not add TUI toast support to Web.
...
| Option                                             | Default              | Meaning                                                                                                                                                                                             |
| -------------------------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tuiQuotaCommandDisplay`                           | `"inline"`           | Choose where native TUI `/quota` appears: `inline` adds an ignored/no-reply plain-text transcript message; `dialog` opens the local popup. `/quota_status` and `/quota_announcements` stay dialogs. |
