...
| Symptom                                                          | Try this                                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/quota` or other slash commands do not appear                   | Confirm `opencode.json` includes `@slkiser/opencode-quota`, then restart OpenCode. The server plugin registers each command once for both TUI and Web/Desktop server; `tui.json` only enables the visual TUI surfaces.                                                                                             |
...
| Popup toasts do not appear in the TUI                            | Check `enableToast`, `showOnIdle`, `showOnQuestion`, and `showOnCompact`. OpenCode 1.17.20 Web does not surface the TUI toast event; Safari/macOS notification permissions do not enable it.                                                                                                                       |
...
| Web shows `Failed to send command` after correct `/quota` output | In OpenCode 1.17.20 this is a false handled-command notification: the deterministic ignored/no-reply output already rendered and no model was called. Do not retry automatically after output appears. The behavior predates v4 and is also reproducible with v3.11.2; it remains an accepted upstream limitation. |
