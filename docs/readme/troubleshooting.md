...
| Symptom                             | Fix                                                                                                                                                                                                                  |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Provider not detected               | Set `OPENCODE_API_KEY`, trusted global `provider.opencode.options.apiKey`, or a strict `opencode-go` API-key entry in OpenCode `opencode.db`; legacy `opencode` is supported only as a fallback. Then check `auth_state`, `auth_source`, and `auth_checked_paths`. |
