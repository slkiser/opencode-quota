...
| `/quota` appears in the wrong TUI location                       | Set `tuiQuotaCommandDisplay` to `"inline"` for the session transcript (default) or `"dialog"` for the local popup, then restart OpenCode. This setting does not change `/quota_status` or `/quota_announcements`.                                                                                             |
