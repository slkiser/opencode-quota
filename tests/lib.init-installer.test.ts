...
describe("init installer planning and merge behavior", () => {
  ...
  it("warns when project-scoped provider mode would be shadowed by global config", async () => {
    ...
    const plan = await planInitInstaller({
      ...
      env: {
        ...process.env,
        HOME: tempDir,
        XDG_CONFIG_HOME: join(tempDir, "global"),
    ...
    expect(
      plan.summaryLines.some((line) =>
        line.includes("Global experimental.quotaToast.enabledProviders will override"),
      ),
    ).toBe(true);
  });
});
